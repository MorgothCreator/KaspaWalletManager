...
static const uint8_t curve_p[NUM_ECC_DIGITS] PROGMEM = CONCAT(Curve_P_, ECC_CURVE);
static const uint8_t curve_b[NUM_ECC_DIGITS] PROGMEM = CONCAT(Curve_B_, ECC_CURVE);
static const EccPoint curve_G PROGMEM = CONCAT(Curve_G_, ECC_CURVE);
static const uint8_t curve_n[NUM_ECC_DIGITS] PROGMEM = CONCAT(Curve_N_, ECC_CURVE);
#define READ_FROM_TABLE(addr)	pgm_read_byte(&addr)
#else*/
...
//#define READ_FROM_TABLE(addr)	addr
//#endif

int ecc::getRandomNumber(uint8_t *p_vli)
{
    //uint64_t _p_vli = 34576437345767453;
    memset(p_vli, 1, 32);
    //memcpy(p_vli, &_p_vli, 8);
    return 1;
}

void ecc::vli_clear(uint8_t *p_vli)
{
    uint i;
...
int ecc::vli_isZero(uint8_t *p_vli)
{
    uint i;
    for(i = 0; i < NUM_ECC_DIGITS; ++i)
    {
...
uint8_t ecc::vli_testBit(uint8_t *p_vli, uint p_bit)
{
    return (p_vli[p_bit/8] & (1 << (p_bit % 8)));
}

/* Counts the number of 8-bit "digits" in p_vli. */
uint ecc::vli_numDigits(uint8_t *p_vli)
{
    int i;
...
uint ecc::vli_numBits(uint8_t *p_vli)
{
    uint i;
    uint8_t l_digit;
...
/* Sets p_dest = p_src. */
void ecc::vli_set(uint8_t *p_dest, uint8_t *p_src)
{
    uint i;
    for(i=0; i<NUM_ECC_DIGITS; ++i)
    {
...
int ecc::vli_cmp(uint8_t *p_left, uint8_t *p_right)
{
    int i;
...
uint8_t ecc::vli_lshift(uint8_t *p_result, uint8_t *p_in, uint p_shift)
{
    uint8_t l_carry = 0;
    uint i;
    for(i = 0; i < NUM_ECC_DIGITS; ++i)
...
void ecc::vli_rshift1(uint8_t *p_vli)
{
    uint8_t *l_end = p_vli;
    uint8_t l_carry = 0;
...
uint8_t ecc::vli_add(uint8_t *p_result, uint8_t *p_left, uint8_t *p_right)
{
    uint8_t l_carry = 0;
    uint i;
    for(i=0; i<NUM_ECC_DIGITS; ++i)
...
uint8_t ecc::vli_sub(uint8_t *p_result, uint8_t *p_left, uint8_t *p_right)
{
    uint8_t l_borrow = 0;
    uint i;
    for(i=0; i<NUM_ECC_DIGITS; ++i)
...
void ecc::vli_mult(uint8_t *p_result, uint8_t *p_left, uint8_t *p_right)
{
    uint16_t r01 = 0;
    uint8_t r2 = 0;
...
void ecc::vli_modAdd(uint8_t *p_result, uint8_t *p_left, uint8_t *p_right, uint8_t *p_mod)
{
...
/* Computes p_result = (p_left - p_right) % p_mod.
   Assumes that p_left < p_mod and p_right < p_mod, p_result != p_mod. */
void ecc::vli_modSub(uint8_t *p_result, uint8_t *p_left, uint8_t *p_right, uint8_t *p_mod)
{
...
void ecc::vli_mmod_fast(uint8_t *p_result, uint8_t *p_product)
{
    uint8_t l_tmp[NUM_ECC_DIGITS];
    int l_carry;
...
void ecc::vli_modMult_fast(uint8_t *p_result, uint8_t *p_left, uint8_t *p_right)
{
...
void ecc::vli_square(uint8_t *p_result, uint8_t *p_left)
{
    uint16_t r01 = 0;
    uint8_t r2 = 0;
...
void ecc::vli_modSquare_fast(uint8_t *p_result, uint8_t *p_left)
{
    uint8_t l_product[2 * NUM_ECC_DIGITS];
...
void ecc::vli_modInv(uint8_t *p_result, uint8_t *p_input, uint8_t *p_mod)
{
...
int ecc::EccPoint_isZero(EccPoint *p_point)
{
...
/* Double in place */
void ecc::EccPoint_double_jacobian(uint8_t *X1, uint8_t *Y1, uint8_t *Z1)
{
    /* t1 = X, t2 = Y, t3 = Z */
...
void ecc::apply_z(uint8_t *X1, uint8_t *Y1, uint8_t *Z)
{
...
/* P = (x1, y1) => 2P, (x2, y2) => P' */
void ecc::XYcZ_initial_double(uint8_t *X1, uint8_t *Y1, uint8_t *X2, uint8_t *Y2, uint8_t *p_initialZ)
{
...
/* Input P = (x1, y1, Z), Q = (x2, y2, Z)
   ...
   or P => P', Q => P + Q
*/
void ecc::XYcZ_add(uint8_t *X1, uint8_t *Y1, uint8_t *X2, uint8_t *Y2)
{
...
/* Input P = (x1, y1, Z), Q = (x2, y2, Z)
   ...
   or P => P - Q, Q => P + Q
*/
void ecc::XYcZ_addC(uint8_t *X1, uint8_t *Y1, uint8_t *X2, uint8_t *Y2)
{
    /* t1 = X1, t2 = Y1, t3 = X2, t4 = Y2 */
    uint8_t t5[NUM_ECC_DIGITS];
...
void ecc::EccPoint_mult(EccPoint *p_result, EccPoint *p_point, uint8_t *p_scalar, uint8_t *p_initialZ)
{
    uint8_t _p_scalar[NUM_ECC_DIGITS];
    bytes2native(_p_scalar, p_scalar);
    /* R0 and R1 */
    ...
    for(i = vli_numBits(_p_scalar) - 2; i > 0; --i)
    {
        nb = !vli_testBit(_p_scalar, i);
        XYcZ_addC(Rx[1-nb], Ry[1-nb], Rx[nb], Ry[nb]);
        XYcZ_add(Rx[nb], Ry[nb], Rx[1-nb], Ry[1-nb]);
    }

    nb = !vli_testBit(_p_scalar, 0);
    XYcZ_addC(Rx[1-nb], Ry[1-nb], Rx[nb], Ry[nb]);
    ...
    native2bytes(p_result->x, Rx[0]);
    native2bytes(p_result->y, Ry[0]);
}

int ecc::make_key(EccPoint *p_publicKey, uint8_t p_privateKey[NUM_ECC_DIGITS], uint8_t p_random[NUM_ECC_DIGITS])
{
...
int ecc::valid_public_key(EccPoint *p_publicKey)
{
...
int ecc::ecdh_shared_secret(uint8_t p_secret[NUM_ECC_DIGITS], EccPoint *p_publicKey, uint8_t p_privateKey[NUM_ECC_DIGITS], uint8_t p_random[NUM_ECC_DIGITS])
{
    EccPoint l_product;
...
/* Computes p_result = (p_left * p_right) % p_mod. */
void ecc::vli_modMult(uint8_t *p_result, uint8_t *p_left, uint8_t *p_right, uint8_t *p_mod)
{
    uint8_t l_product[2 * NUM_ECC_DIGITS];
...
int ecc::ecdsa_sign(uint8_t p_privateKey[NUM_ECC_DIGITS], uint8_t p_hash[NUM_ECC_DIGITS], uint8_t p_signature[NUM_ECC_DIGITS*2])
{
    ...
    /* tmp = k * G */
    native2bytes(l_tmp, k);
    EccPoint_mult(&p, &curve_G, l_tmp, NULL);
    bytes2native(l_tmp, p.x);
    ...
    bytes2native(l_tmp, p_privateKey);
    vli_modMult(s, r, l_tmp, curve_n); /* s = r*d */
    bytes2native(l_tmp, p_hash);
    ...
    native2bytes(p_signature, r);
    native2bytes(p_signature + NUM_ECC_DIGITS, s);

    return 1;
}

int ecc::ecdsa_verify(EccPoint *p_publicKey, uint8_t p_hash[NUM_ECC_DIGITS], uint8_t r[NUM_ECC_DIGITS], uint8_t s[NUM_ECC_DIGITS])
{
...
void ecc::bytes2native(uint8_t p_native[NUM_ECC_DIGITS], uint8_t p_bytes[NUM_ECC_DIGITS*4])
{
    unsigned i;
    for(i=0; i<NUM_ECC_DIGITS; ++i)
    {
...
void ecc::native2bytes(uint8_t p_bytes[NUM_ECC_DIGITS*4], uint8_t p_native[NUM_ECC_DIGITS])
{
    unsigned i;
    for(i=0; i<NUM_ECC_DIGITS; ++i)
    {
